...
async function main() {
  ...
  if (HASHES.tx1) await analyze(HASHES.tx1 as `0x${string}`, abi);
  if (HASHES.tx2) await analyze(HASHES.tx2 as `0x${string}`, abi);
  if (HASHES.tx3) await analyze(HASHES.tx3 as `0x${string}`, abi);
}
